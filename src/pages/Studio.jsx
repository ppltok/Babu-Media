...
export default function Studio() {
  ...
  return (
    <div className={`h-screen flex flex-col lg:flex-row overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#0B0A16] text-white' : 'bg-gray-50 text-gray-900'}`}>
        ...
        <div className={`lg:hidden fixed inset-0 top-16 z-40 flex flex-col ${isDark ? 'bg-[#0d0c18]' : 'bg-white'}`}>
          ...
          <div className={`p-4 border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
            ...
            >
              ...
              <span>{isDark ? (isRTL ? 'מצב בהיר' : 'Light Mode') : (isRTL ? 'מצב כהה' : 'Dark Mode')}</span>
            </button>
            <button
              onClick={() => {
                navigate('/settings')
      ...
      {/* Floating Theme Toggle - Top Right (hidden on mobile to avoid overlap with header) */}
      <button
        onClick={toggleTheme}
        className={`hidden lg:flex fixed top-4 z-40 w-10 h-10 rounded-full items-center justify-center shadow-lg transition-all hover:scale-110 ${
...
function PlotWorldContent({ childId, child, initialCharacter, onCharacterUsed, user }) {
  ...
  const getStoryGradient = (storyId) => {
    ...
    // Use sum of char codes to get a consistent index
              ...
              <div className={`${bookInnerClass} rounded-2xl overflow-hidden shadow-inner flex flex-col`}>
                  ...
                  {currentSpread.type === 'end' ? (
                    ...
                    <div className={`flex flex-col ${BOOK_CONTENT_HEIGHT}`}>
                      <div className={`flex-1 flex flex-col md:flex-row items-center justify-center ${isRTL ? 'md:flex-row-reverse' : ''}`}>
